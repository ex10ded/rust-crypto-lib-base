...
// NOTE: The old `get_extended_chain_{order,cancel,set_market_leverage,
// create_account}_hash_bytes[_with_domain_hash]` functions were removed.
// They used the XC body-hash wrapping pattern (Poseidon("XC_ORDER", ...))
// which is no longer used for structured txs. The validator's tx_auth.rs
// now computes struct.hash() → OffChainMessage directly. The WASM crate
// does the same. Only admin txs (RegisterAsset, PriceUpdate, etc.) still
// use the body-hash pattern via `get_extended_chain_payload_hash_bytes`.
